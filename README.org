...
#+BEGIN_SRC emacs-lisp
  (let ((stuff_files (list "/home/temp/my-stuffs01.org"
    ...
    (let ((entropy/cs-entry-recorde-file chosen_file)
          (entropy/cs-cached-plist  nil))
      (entropy/cs-open-all)))
#+END_SRC 

With the obviously another notice following the 'pointer' file let
form, cache list =entropy/cs-cached-plist= must be cleaned out before
the func =entropy/cs-open-all= using. There what you must notice that
variabnle =entropy/cs-cached-plist= was the second part of the
...
sticking variable =entropy/cs-cached-plist= whose value must
...
#+BEGIN_SRC elisp
  (let ((entropy/cs-entry-recorde-file "xxxx")
        (entropy/cs-cached-plist (entropy/cs-get-cached-list)))
