...
directory , all of them was implementation independently but which
that some of the functions and variables built in them are may shared
accross to sets of =entropy-emacs-xxx.el= files, as the result, I make
the =entropy-emacs-defcustom.el= and =entropy-emacs-const.el= for
storing the Cross-files variables for making the convenience for
tidying up or modifying config. And other =entropy-emacs-xxx.el= file
are reference the public variable from them to genertate the current
emacs session config state.
...
| entropy-project-manager         | project managements extension                                                                     |
...
+ *Derived slot create:*
  ...
  =main-slot.derived_number:⛓main-slot:"main workspace
  tagname"☛derived name=
  ...
  =entropy/emacs-basic-eyebrowse-switch-top=, it's useful for quicly
  back from the temp derived slot to top (called 'top' by main) slot
  without remember the current main slot number or backing with
  calling =entropy/emacs-basic-eyebrowse-switch-basic-window=.
...
was given as function ~entropy/emacs-c-win-c-compiler~ which using
mingw gcc for the compiler and with fixed library linking as for.
